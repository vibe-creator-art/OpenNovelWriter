import { type SVGProps, useId } from 'react'

import type { IconComponent } from '../../types'
const ClaudeCodeLight: IconComponent = (props: SVGProps<SVGSVGElement>) => {
  const iconId = useId()

  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="none" viewBox="0 0 120 120" {...props}>
      <g clipPath={`url(#${iconId}-claudecodelight__a)`}>
        <path
          fill="#D97757"
          fillRule="evenodd"
          d="M91.7428 55.7339H102.5V66.8494H91.75V77.6998H86.4216V88.1667H81V77.6998H75.6716V88.1667H70.25V77.6998H48.75V88.1667H43.332V77.6998H38V88.1667H32.5784V77.6998H27.25V66.8458H16.5V55.7375H27.25V34.4167H91.7428V55.7339ZM38 55.7339H43.332V45.5322H38V55.7339ZM75.6608 55.7339H81V45.5322H75.6608V55.7339Z"
          clipRule="evenodd"
        />
      </g>
      <defs>
        <clipPath id={`${iconId}-claudecodelight__a`}>
          <path fill="#fff" d="M0 0H86V86H0z" transform="translate(16.5 16.5)" />
        </clipPath>
      </defs>
    </svg>
  )
}
export { ClaudeCodeLight }
export default ClaudeCodeLight
