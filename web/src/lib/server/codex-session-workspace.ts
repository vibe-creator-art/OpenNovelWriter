...
async function writeSessionAgentsFile(sessionPath: string, ownerId: string) {
    ...
    const parts = [
        ...
        '',
        '- Read generated novel context from `novel/`; its Markdown files are read-only symlinks, including nested directories such as `novel/chapters/`, `novel/terms/`, `novel/snippets/`, and `novel/story-state/`.',
        '- `novel/outline.md` is the story outline and index, `novel/chapters/` contains one Markdown file per chapter keyed by chapter id, `novel/terms/` contains one Markdown file per term keyed by term title (disambiguated with a `-2`/`-3` suffix when titles collide), `novel/snippet.md` indexes author snippets, and `novel/story-state/` projects explicit Moments, entities, and currently credible facts.',
        ...
        '- Image artifact references use `[label](image:<manifest-path>#<optional-item-id>)`. Read the manifest under `artifacts/` and use the selected item file when a request refers to one. For image generation and editing, prefer the native `imagegen` skill and `image_gen` tool; if the current connection cannot use them, fall back to `onw-imagegen`. Show `generate_images`\' returned `suggestedLink` on its own line.',
        ...
        '- Mutating OpenNovelWriter data goes through MCP tools, never by editing projections. Read the matching built-in skill first: `edit-manuscript` for novel/act/chapter titles and scene prose, `edit-summary` for scene and act/volume summaries, `edit-snippets` for snippets, `edit-terms` for terms and term-gallery image uploads, `edit-prompts` for creating/editing/deleting prompts or converting external prompt presets, and `story-state` for Moments, entities, aliases, facts, and source Episodes.',
        '- To recall earlier manuscript events, locate relevant scenes, or check story continuity, read the built-in `story-context-retrieval` skill and use its read-only retrieval tool. Set its knowledge-graph option only when Story State candidates are useful.',
        ...
        '- Two kinds of skills exist here. Built-in skills (`edit-manuscript`, `edit-summary`, `edit-snippets`, `edit-terms`, `edit-prompts`, `edit-skills`, `story-state`) appear in your skills catalog as usual — read their SKILL.md before mutating OpenNovelWriter data. User skills are delivered by explicit injection instead: when the user invokes one with its canonical `/skill-name` command it is attached to this turn as a `$skill` carrying its full SKILL.md instructions (and, for prompt-backed skills, a pre-assembled artifact). An invoked user skill may NOT show up in your own `skills/list` catalog — that is expected and fine; act on the injected `$skill` instructions you were given, do not doubt it or try to locate the skill in your local skills list.',
