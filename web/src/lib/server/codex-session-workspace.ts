...
async function writeSessionAgentsFile(sessionPath: string, ownerId: string) {
    ...
    const parts = [
        ...
        '- Mutating novel data goes through MCP tools, never by editing `novel/` projections. Read the matching built-in skill first: `edit-manuscript` for titles/scene summaries/scene prose, `edit-snippets` for creating/editing/deleting snippets, `edit-terms` for creating/editing/deleting terms (characters, locations, items, …, including their experiences timeline).',
