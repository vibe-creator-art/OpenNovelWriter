...
async function writeSessionAgentsFile(sessionPath: string, ownerId: string) {
    ...
    const parts = [
        ...
        '- Two kinds of skills exist here. Built-in skills (`edit-manuscript`, `edit-summary`, `edit-snippets`, `edit-terms`, `edit-prompts`, `edit-skills`) appear in your skills catalog as usual — read their SKILL.md before mutating OpenNovelWriter data. User skills are delivered by explicit injection instead: when the user invokes one with its canonical `/skill-name` command it is attached to this turn as a `$skill` carrying its full SKILL.md instructions (and, for prompt-backed skills, a pre-assembled artifact). An invoked user skill may NOT show up in your own `skills/list` catalog — that is expected and fine; act on the injected `$skill` instructions you were given, do not doubt it or try to locate the skill in your local skills list.',
