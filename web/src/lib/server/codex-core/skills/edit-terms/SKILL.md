---
name: edit-terms
description: 创建、修改、删除小说词条，或把已有图片上传到词条画廊（角色/地点/物品/设定等词条卡，含别名、描述、经历时间线、关系、标签、分类、颜色）。当用户要求新增 NPC、地点、物品、技能等设定，更新人物设定、经历或关系，重命名、归类、删除词条或向词条画廊添加图片时，先读本技能再调用 create_term / edit_term / delete_term / upload_term_gallery_image。只是查阅词条内容时不需要本技能，直接读 novel/terms/ 下的投影文件。
---
...
## upload_term_gallery_image — 上传图片到画廊

用户明确要求把一张已有图片加入某个词条画廊时调用。这个工具**只上传并追加图片**，不会生成或编辑图片，不会删除已有画廊内容，也不会把图片设为词条头像。

- 传 `novelId`、目标词条投影中的 `termId`，以及图片文件的绝对路径 `imagePath`。
- 图片必须位于当前 Codex 会话的 `artifacts/` 目录中，格式为 JPEG、PNG、GIF 或 WebP，大小不超过 5 MB。
- `onw-imagegen` 生成的图片可以从 manifest 中取得实际文件路径后直接上传；不要把 manifest JSON 本身传给工具。
- 原生 `imagegen` 生成的文件若仍在 `$CODEX_HOME/generated_images/`，先把用户选中的最终图片复制到当前会话的 `artifacts/image-inputs/`，再调用本工具；不要移动或覆盖原图。
- 只有用户要求“加入画廊”“保存到角色/地点/物品画廊”等动作时才上传。单纯生成或展示图片时不要擅自写入词条。
