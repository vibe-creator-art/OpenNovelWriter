...
- `scene_continuation`：使用当前指令、指令提及词条、前文/后文、故事摘要、卷纲/章纲和用户明确需要的资料选择。
...
3. 为真正需要频繁调整的参数创建输入：二元设置用 checkbox，互斥选项用单选 dropdown，可叠加选项才用多选。外部预设迁移时，输入必须对应源预设中的候选项或用户明确提出的参数，不能自行补充。
