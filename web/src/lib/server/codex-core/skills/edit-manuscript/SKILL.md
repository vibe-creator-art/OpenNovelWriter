...
- 正文投影只保留两种行内格式：`**粗体**` 和 `*斜体*`。复制 `old_text` 时保留其中的格式标记；`new_text` 也可以使用这两种标记，系统会转换回正文格式。不要引入其他 Markdown 格式。
- `old_text` 为空字符串表示在场景**末尾追加** `new_text`——这也是往**空场景起笔**的唯一方式（空场景没有可匹配的锚点）。
- 动手前**先读 `novel/chapters/<chapter_id>.md`**，把 `old_text` 从正文里**逐字照抄**（含标点和粗体/斜体标记）。`old_text` 必须在该场景里唯一，不唯一就多带些上下文。
