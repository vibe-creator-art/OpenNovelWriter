...
const CATEGORY_TRANSLATION_KEY: Record<string, string> = {
    scene_continuation: 'sceneContinuation',
    scene_action: 'sceneAction',
    ai_chat: 'aiChat',
}

type SkillPresetLibrarySectionProps = {
...
export function SkillPresetLibrarySection({
    presets,
    loading,
    error,
    cloningPresetId,
    ...
    className,
}: SkillPresetLibrarySectionProps) {
    const t = useTranslations('skills')
    const visiblePresets = useMemo(() => presets.filter((preset) => preset.skillCount > 0), [presets])
    ...
    const categoryLabel = (category: string) => {
        ...
        const key = normalized ? CATEGORY_TRANSLATION_KEY[normalized] : null
        ...
        <>
                                        ...
                                        <div className="mt-2 flex items-center justify-between gap-2">
                                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                                                <Badge variant="outline" className="font-normal">
                                                    {categoryLabel(preset.entrySkillCategory)}
                                                </Badge>
                                                <span className="min-w-0 truncate text-[11px] text-muted-foreground">
                                                    {preset.entrySkillName}
                                                </span>
                                            </div>
                                            <Button
                                                size="sm"
            ...
            <AlertDialog open={cloneOverwriteConfirmOpen} onOpenChange={onCloneOverwriteConfirmOpenChange}>
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={cloningPresetId !== null}>{t('presets.cancel')}</AlertDialogCancel>
